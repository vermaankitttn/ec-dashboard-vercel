...
# Remove auto-refresh meta tag to prevent interference with tab switching
# st.markdown("""
...
if 'last_tab_switch' not in st.session_state:
    st.session_state.last_tab_switch = datetime.now()
...
def get_countdown_text():
    ...
    return f"Auto-refresh in {seconds}s (every 10s)"
...
def main():
    ...
    # Auto-refresh configuration - fixed at 1 minute
    refresh_interval = 1.0  # 1 minute
    
    # Hardcoded Google Sheets configuration
    st.sidebar.subheader("🔗 Google Sheets Connection")
    
    # Fixed credentials file and spreadsheet URL
    credentials_file = "ec-results-credentials.json"
    spreadsheet_url = "https://docs.google.com/spreadsheets/d/1wQrOseeikbaJnF3_twiuI-FmSKo9tLTly02eFTCCLGQ/edit?gid=1422280249#gid=1422280249"
    
    # Load data directly
    current_data = load_google_sheets_data(credentials_file, spreadsheet_url)
    
    if current_data is not None:
        st.sidebar.markdown('<div class="status-indicator connected">✅ Connected to Google Sheets</div>', unsafe_allow_html=True)
        st.sidebar.markdown(f"📊 **Total Candidates:** {len(current_data)}")
    else:
        st.sidebar.markdown('<div class="status-indicator disconnected">❌ Connection Failed</div>', unsafe_allow_html=True)
        st.sidebar.error("Failed to load data from Google Sheets. Please check the credentials file and URL.")
    ...
    if current_data is not None:
        ...
        # Remove auto-refresh logic to prevent interference with tab switching
        ...
        #     st.session_state.next_refresh_time = now + timedelta(minutes=1)
        ...
        # Create tabs for top 10 and remaining candidates
        tab1, tab2 = st.tabs(["🏆 Top 10 Candidates", "📊 Remaining Candidates (11+)"])
        ...
        with tab1:
            ...
            for idx, (_, row) in enumerate(current_data.iterrows()):
                if idx >= 10:  # Limit to top 10 candidates
                    break
        ...
        with tab2:
            ...
            # Display remaining candidates (positions 11 and beyond)
            remaining_candidates = current_data.iloc[10:]  # Start from index 10 (position 11)
            
            for idx, (_, row) in enumerate(remaining_candidates.iterrows()):
                if idx >= 21:  # Limit to 21 candidates (3x7 grid)
                    break
                    
                candidate_name = row.get('Candidate Name', f'Candidate {idx+11}')  # Position starts from 11
                image_url = row.get('Image', None)  # Get image URL if available
                
                # Determine position change
                change = position_changes.get(candidate_name, 'same')
                
                # Calculate actual position (11, 12, 13, etc.)
                actual_position = idx + 11
        ...
        if st.session_state.last_refresh:
            ...
            st.sidebar.markdown("⏰ **Auto-refresh:** Every 1 minute")
